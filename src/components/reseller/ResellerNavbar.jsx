...
export default function ResellerNavbar() {
  ...
  const [currentUsername, setCurrentUsername] = useState(getAuthUsername() || "Mitra Reseller");
  ...
  const username = currentUsername || "Mitra Reseller";
  ...
  return (
    <nav
      ...
      style={{
        ...
        borderBottom: "1px solid rgba(0,187,51,0.2)",
    ...
    >
      <div className="w-full max-w-[1536px] mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-[58px] gap-2">
          ...
          <Link to="/reseller/dashboard" className="flex items-center gap-2.5 shrink-0 whitespace-nowrap">
            ...
            <span
              ...
              style={{
                background: "rgba(0,187,51,0.12)",
                border: "1px solid rgba(0,187,51,0.3)",
                color: MP_GREEN,
              }}
            >
              Mitra Portal
          ...
          {/* Center: Desktop Nav Links (Sejajar horizontal, persis seperti ViewerNavbar) */}
          <div className="hidden lg:flex items-center gap-1 xl:gap-1.5 shrink-0 flex-nowrap">
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.path}
                  to={link.path}
                  className="flex items-center gap-1.5 px-2.5 xl:px-3 py-1.5 rounded-xl text-xs font-semibold whitespace-nowrap shrink-0 transition-all duration-200"
                  style={{
                    background: isActive ? MP_GREEN : "transparent",
                    color: isActive ? "white" : "rgba(255,255,255,0.6)",
                    boxShadow: isActive ? `0 0 16px ${MP_GREEN}44` : "none",
                  }}
                  onMouseEnter={(e) => {
                    if (!isActive) {
                ...
                >
                  <Icon size={14} style={{ color: isActive ? "white" : MP_GREEN }} />
                  <span>{link.name}</span>
                </Link>
              );
            })}
          ...
          {/* Right: Controls (Search + Points Wallet + Profile) */}
          <div className="hidden md:flex items-center gap-2.5 shrink-0 whitespace-nowrap">
            ...
            >
              ...
              <span className="whitespace-nowrap">Cari fitur...</span>
            </button>

            {/* Quick M-Point Indicator */}
            <Link
              to="/mpoint"
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl transition text-xs font-bold shrink-0 whitespace-nowrap"
              style={{
                background: "rgba(0,187,51,0.12)",
                border: "1px solid rgba(0,187,51,0.3)",
                color: MP_GREEN,
            ...
            >
              <Coins size={14} className="animate-pulse shrink-0" />
              <span>Poin Saya</span>
            </Link>
            ...
            <div className="relative shrink-0">
              ...
              >
                ...
                <span className="text-xs font-semibold text-white hidden sm:block truncate max-w-[85px]">
                  {username}
                </span>
      ...
      {mobileMenuOpen && (
        <div
          ...
          style={{ borderColor: "rgba(0,187,51,0.15)", background: "rgba(10,13,20,0.98)" }}
        >
          {navLinks.map((link) => {
            const Icon = link.icon;
            const isActive = location.pathname === link.path;
            return (
              <Link
                ...
                style={{
                  background: isActive ? MP_GREEN : "rgba(255,255,255,0.03)",
                  color: isActive ? "white" : "rgba(255,255,255,0.6)",
                }}
              >
                <Icon size={16} style={{ color: isActive ? "white" : MP_GREEN }} />
                <span>{link.name}</span>
              </Link>
            );
          })}
          ...
          >
            <div className="flex items-center gap-2">
              {customAvatar ? (
                ...
                <div
                  className={`w-7 h-7 rounded-lg font-black text-xs flex items-center justify-center text-white shrink-0 bg-gradient-to-tr ${avatarPreset}`}
                >
                  {(username || "M").charAt(0).toUpperCase()}
                </div>
              )}
              <div>
                ...
                <p className="text-[10px]" style={{ color: MP_GREEN }}>Mitra Reseller</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-3 py-1.5 rounded-xl text-xs font-bold text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 cursor-pointer"
              >
                Profil
              </button>
              <button
