...
export default function InteraksiPage() {
  ...
  const handleExportCSV = () => {
    exportToCSV(interaksiList, "data_referral_reseller", [
      { key: "id", label: "ID" },
      { key: "nama_reseller", label: "Nama Reseller Perujuk" },
      { key: "nama_agen", label: "Nama Agen / Downline" },
      { key: "tanggal_gabung", label: "Tanggal Bergabung" },
      { key: "komisi", label: "Total Komisi (Rp)" },
      { key: "status", label: "Status Referral" },
    ]);
  };
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">
              Pantau pohon relasi rujukan (*reference referral*) antara reseller dan agen mitra
            </p>
          </div>

          <button
            onClick={handleExportCSV}
            className="px-3.5 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold rounded-xl shadow-md transition flex items-center gap-2 cursor-pointer self-start sm:self-auto"
          >
            <Download size={15} />
        ...
        {loading ? (
          ...
          <div className="p-12 text-center bg-white rounded-3xl border border-slate-200 shadow-xs space-y-3">
            ...
            <p className="text-xs text-slate-400 max-w-sm mx-auto">
              Tidak ada relasi referral yang sesuai dengan pencarian Anda.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
